...
class GINConvEdge(MessagePassing):
    def __init__(this, in_dim, emb_dim, net_norm="none"):
        ...
        this.bond_encoder = nn.Linear(3, emb_dim)#BondEncoder(emb_dim=emb_dim)
        this.mlp_edge_attr = nn.Linear(in_dim + emb_dim, emb_dim)
        this.mlp_x = nn.Linear(in_dim, emb_dim)

    def forward(this, x, edge_index, edge_attr):
        edge_attr = edge_attr.float()
        edge_embedding = this.bond_encoder(edge_attr)
        out = this.mlp(
            (1 + this.eps) * this.mlp_x(x)
            + this.propagate(
                edge_index,
                x=x,
    ...
    def message(this, x_j, edge_attr):
        ...
        return F.relu(concat_j_edge)#x_j + edge_attr)
...
class GCNConvEdge(MessagePassing):
    def __init__(this, in_dim, emb_dim):
        super(GCNConvEdge, this).__init__(aggr="add")
        this.linear = nn.Linear(in_dim, emb_dim)
...
class GNN_node(nn.Module):
    def __init__(
        ...
        for layer in range(num_layer):
            if layer == 0:
                if gnn_type == "GINConv":
                    this.convs.append(GINConvEdge(9, emb_dim, this.net_norm))
                elif gnn_type == "GCNConv":
                    this.convs.append(GCNConvEdge(9, emb_dim))
                elif gnn_type == "GATConv":
                    this.convs.append(GATConv(9, emb_dim))
                elif gnn_type == "SAGEConv":
                    this.convs.append(SAGEConv(9, emb_dim))
                else:
                    raise ValueError(f"Undefined GNN type: {gnn_type}")
                this.batch_norms.append(nn.BatchNorm1d(emb_dim))
            else:
                if gnn_type == "GINConv":
                    this.convs.append(GINConvEdge(emb_dim, emb_dim, this.net_norm))
                elif gnn_type == "GCNConv":
                    this.convs.append(GCNConvEdge(emb_dim, emb_dim))
                elif gnn_type == "GATConv":
    ...
    def forward(this, x, edge_index, edge_attr, batch):
        ...
        if this.JK == "last":
            ...
            for layer in range(1, this.num_layer + 1):
