...
class GINConvEdge(MessagePassing):
    def __init__(this, emb_dim, net_norm="none"):
        ...
        this.bond_encoder = nn.Embedding(2, emb_dim)
    ...
    def message(this, x_j, edge_attr):
        concat_j_edge = torch.cat((x_j, edge_attr.squeeze()), 1)
...
class GCNConvEdge(MessagePassing):
    ...
    def forward(this, x, edge_index, edge_attr):
        edge_embedding = this.bond_encoder(edge_attr.reshape(-1))
        x = this.linear(x)
        row, col = edge_index
...
class GNN_node(nn.Module):
    def __init__(
        ...
        this.atom_encoder = nn.Embedding(100, emb_dim)
    ...
    def forward(this, x, edge_index, edge_attr, batch):
        x = x.long()
        h_list = [this.atom_encoder(x.reshape(-1))]
