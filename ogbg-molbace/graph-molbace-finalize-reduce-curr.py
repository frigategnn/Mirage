...
dataset[split_idx["train"]].y.sum(), len(dataset[split_idx["train"]].y)
...
# ---
# **Steps:**
# 1. Enumerate all trees of depth $k$ from each node.
# 1. During the enumeration, map each tree to a canonical label (DFSCode). (This should be polynomial time.)
# 1. Hash the DFS codes to compute frequency of each tree.
# 1. Retain top $X\%$.
...
counter_index = 0 
for index_graph, data in enumerate(train_dataset_processed[0:]): #enumerate(dataset[0:train_upto]):
    counter_index+=1
...
len(map_label_to_mptree)


# In[41]:


len(labels_trees_to_graph_dict)


# In[42]:


len(labels_trees_to_graph_dict.keys())
...
# len(tree_ctr_class[0]), len(tree_ctr_class[1])
...
len(transactions_classwise[classid][graphid])
...
len(transactions_array_classwise[0]), len(transactions_array_classwise[1])
...
# # 75% 1:6
# threshold_class_dict  = {0:105,1:70
#                         }


# threshold_class_dict  = {0:104,1:72
                         
                         
#                         }

# # threshold_class_dict  = {0:110,1:80
...
gnn_type = 'GCNConv'
#         model = GCN(reduce_type= 'sum' , JK='sum', net_norm ='none', hidden_channels=256, num_layer=2,
#                     gnn_type='GATConv',drop_ratio=0.1).to(device)
#         optimizer = torch.optim.Adam(model.parameters(), lr=0.001,  weight_decay=1e0)
...
for gnn_type in ['GATConv', 'GCNConv', 'GINConv']:
    ...
    threshold_class_dict={0:109, 1:79}
    threshold_class_dict={0:105, 1:72}
    ...
    for step in range(num_steps): 
        ...
        print('len(patterns_dict[0]),len(patterns_dict[1])', len(patterns_dict[0]),len(patterns_dict[1]))
        ...
        print(len(set(p1_keys).difference(p0_keys)))
        ...
        random.shuffle(graphs_py)#.shuffle()

        train_dataset = graphs_py#dataset[:train_upto]
        ...
        for train_data_obj in train_dataset:
            # print('train_data_obj', train_data_obj)
        ...
        print('nodes X features', num_nodes*num_features)
    ...
    #                 if best_val_auc_across_step< val_acc:
    #                     best_val_auc_across_step= val_acc
    #                     best_test_auc_at_best_val_across_step = test_acc
