...
class tree_node{
    public:
        int label, idx, original_label;
        std::vector<std::pair<int, int>> children;
        tree_node(const int &lab, const int &idx_, const int &orig){
            label = lab;
            idx = idx_;
            original_label = orig;
        }
...
std::string get_can_lab_tree(const std::vector<tree_node*> &tree, int root=0){
    ...
    std::stringstream ss;
    ss << tree[root]->label << " " << tree[root]->original_label;
...
void get_canonical_labels(const BGraph &G,
        ...
        const int &n_hops,
        const std::vector<int> &node_attrs_original){
    /* 1. Create tree for each node.
     * 1.1 Renumber nodes while creating tree, and remap them to correct
     *     node_attr and correct edge_attr. (Just maintain a map from
     *     original to new node id, then use it as interface between the
     *     old and new maps.)
    ...
    for(boost::graph_traits<BGraph>::vertex_iterator it=vbegin; it!=vend; it++){
        ...
        tree.push_back(new tree_node(node_attrs[*it], node_ctr, node_attrs_original[*it]));
...
std::vector<std::string> data_to_BGraph(py::object data, int n_hops){
    ...
    py::object original_x = data.attr("original_x");
    py::object edge_attr = data.attr("edge_attr");
    py::object node_attr = data.attr("node_attr");
    py::object original_edge_attr = data.attr("original_edge_attr");
    ...
    std::vector<int> node_attrs_original;
    for(auto el : x){
        py::int_ item_ = el.attr("item")();
        int itm_ = item_;
        node_attrs.push_back(itm_);
    }
    for(auto el : node_attr){
        py::int_ item_ = el.attr("item")();
        int itm_ = item_;
        node_attrs_original.push_back(itm_);
    }
    ...
    get_canonical_labels(G, node_attrs, edge_attrs, edge_pair_to_ind,
            canonical_labels, n_hops, node_attrs_original);
    return canonical_labels;
}
py::list canonical(py::list list, py::int_ n_hops){
    /* This function gets a list of PyTorch Geometric Data Objects that
     * contain `x`, `edge_index`, `original_x`, `edge_attr`, `y`,
     * `original_edge_attr`. Each of which are tensors of long.
     *
    ...
    int n_hops_ = n_hops;
    for(auto item : list){
        canonical_labels = data_to_BGraph(py::reinterpret_borrow<py::object>(item), n_hops_);
